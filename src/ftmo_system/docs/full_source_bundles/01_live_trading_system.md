...
class LiveTradingSystemV5:
    ...
    def __init__(
        ...
        # Load system config (for simulation_mode and other flags)
        self.config: dict = {}
        try:
            config_path = self._system_root / "config" / "ftmo_config.json"
            with open(config_path, "r") as f:
                self.config = json.load(f)
        except Exception:
            self.config = {}
        self.simulation_mode: bool = self.config.get("mode", {}).get("simulation_mode", False)
        ...
        # In simulation_mode, trade all sessions (incl. Asian) — more signals
        # = more labeled training data, and no real capital is at risk.
        self.hard_filters = HardFilters(
            min_atr_pips=8.0,
            news_buffer_minutes=30,
            require_liquid_session=not self.simulation_mode
        )
        ...
        # Initialize feature history recorder (per-cycle 27-feature snapshots
        # for sequence models — LSTM/Transformer/CNN)
        from core.feature_history_recorder import FeatureHistoryRecorder
        _feature_history_dir = str(self._system_root / "data" / "feature_history")
        self.feature_history = FeatureHistoryRecorder(_feature_history_dir)
        ...
        if HAS_AZURE:
            ...
            if self.azure.enabled:
                self.log_system("[AZURE] Bridge connected — Colab predictions will be used if/when fresh")
            else:
                self.log_system("[AZURE] Bridge disabled — running local sources only")
        else:
            self.azure = None

        # Track Colab prediction freshness — predictions go stale if the
        # Colab runtime disconnects, but get_prediction() fails silently.
        self._colab_start_time = datetime.now()
        self._last_colab_prediction_time = None
        self._colab_stale_warned = False
        self._COLAB_STALE_WARN_MINUTES = 10
    ...
    def extract_features(self, data=None):
        ...
        for pair in self.pairs:
            ...
            try:
                ...
                # Record raw 27-feature snapshot for sequence-model training
                self.feature_history.append(pair, base_features)
    ...
    def _run_strategy_engine(self, features_dict: dict):
        ...
        if self.azure and self.azure.enabled:
            try:
                ...
                if colab_preds:
                    self._last_colab_prediction_time = datetime.now()
                    self._colab_stale_warned = False
                else:
                    reference = self._last_colab_prediction_time or self._colab_start_time
                    stale_minutes = (datetime.now() - reference).total_seconds() / 60.0
                    if stale_minutes >= self._COLAB_STALE_WARN_MINUTES and not self._colab_stale_warned:
                        if self._last_colab_prediction_time is None:
                            self.log_system(
                                f"[COLAB] WARNING: no fresh predictions received in "
                                f"{stale_minutes:.0f} min since startup — "
                                f"Colab runtime may be disconnected"
                            )
                        else:
                            self.log_system(
                                f"[COLAB] WARNING: no fresh predictions for {stale_minutes:.0f} min "
                                f"(last seen {self._last_colab_prediction_time.isoformat()}) — "
                                f"Colab runtime may have disconnected"
                            )
                        self._colab_stale_warned = True
    ...
    def check_ftmo_drawdown_safety(self, current_balance: float) -> bool:
        ...
        In simulation_mode, no real capital is at risk (signals are paper-traded
        and labeled via tick replay), so the drawdown halt is bypassed entirely
        to keep signal generation — and therefore training data collection —
