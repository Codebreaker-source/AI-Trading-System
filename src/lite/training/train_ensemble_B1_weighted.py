#!/usr/bin/env python3
"""
B1 Ensemble Training Script with Class Weighting
File: train_ensemble_B1_weighted.py
Version: 1.0
Date: 2025-11-03

Configuration: B1 (24 candles, 10 pips lookback)
Purpose: Train 24 ensemble models with class weighting to handle 94% HOLD imbalance
Data Format: 61 columns (timestamp, symbol, 58 features, label)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('training_B1_weighted_log.txt'),
...
class Config:
    """B1 Training configuration (24 candles, 10 pips) - MEMORY OPTIMIZED"""
    
    # Paths - B1 SPECIFIC - UPDATED FOR OFFSET DATA (NO LEAKAGE)
    BASE_DIR = Path(r"C:\Users\mt5-admin\Documents\TradingSystem\AzureDeploy\Phase4_LITE_System")
    TRAIN_FILE = BASE_DIR / "training" / "train_data_24c_10p_offset.csv"
    VAL_FILE = BASE_DIR / "training" / "val_data_24c_10p_offset.csv"
    TEST_FILE = BASE_DIR / "training" / "test_data_24c_10p_offset.csv"
    MODEL_DIR = BASE_DIR / "trained_models_B1_CLEAN"
...
class ChunkedDataLoader:
    ...
    def load_pair_data_chunked(self, file_path: Path, pair: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info(f"{pair}: {len(X):,} samples loaded")
...
class XGBoostTrainer:
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        hold_count = len(y_train[y_train == 0])
        action_count = len(y_train[y_train != 0])
...
class EnsembleTrainer:
    ...
    def train_all_models(self):
        ...
        logger.info("="*80)
        logger.info("B1 ENSEMBLE TRAINING - 24 MODELS WITH CLASS WEIGHTING")
        logger.info("Configuration: 24 candles, 10 pips")
        logger.info("="*80)
        ...
        logger.info("\n" + "="*80)
        logger.info("B1 TRAINING COMPLETE")
        logger.info("="*80)
...
def main():
    ...
    try:
        ...
        logger.info("="*80)
        logger.info("B1 ENSEMBLE TRAINING SCRIPT")
        logger.info("Configuration: 24 candles, 10 pips")
        logger.info("="*80)
