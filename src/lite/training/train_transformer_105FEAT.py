#!/usr/bin/env python3
"""
Train 8 Transformer Models on 105 Features - ALL PROVEN FIXES
==============================================================
File: train_transformer_105FEAT.py
Date: 2025-11-29

FIXES APPLIED (from Nov 15-17 chat history):
1. .dropna() before training - removes NaN rows
2. Full batch training (not mini-batch)
3. Class weights (inverse frequency, ~9-15x)
4. Learning rate 0.001
5. Memory-safe chunked loading

Models: 8 Transformer (one per currency pair)
"""
...
# Setup logging
log_file = TRAINING_DIR / f"training_TRANSFORMER_105FEAT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
# ============================================================================
# MODEL DEFINITION - SimpleTransformer
# ============================================================================
...
def main():
    print("="*80)
    print("TRAINING 8 TRANSFORMER MODELS - 105 FEATURES (ALL FIXES APPLIED)")
    print("="*80)
    logger.info("="*80)
    logger.info("TRANSFORMER TRAINING - 105 FEATURES")
    logger.info("="*80)
    logger.info(f"Train file: {TRAIN_FILE}")
    logger.info(f"Val file: {VAL_FILE}")
    logger.info(f"Output: {OUTPUT_DIR}")
    logger.info("Fixes: .dropna(), full batch, class weights, LR 0.001")
    logger.info("="*80)
    ...
    for i, symbol in enumerate(SYMBOLS, 1):
        print(f"\n{'='*80}")
        print(f"[{i}/8] TRAINING TRANSFORMER: {symbol}")
        print("="*80)
        logger.info(f"\n{'='*80}")
        logger.info(f"[{i}/8] TRAINING TRANSFORMER: {symbol}")
        logger.info("="*80)
        ...
        try:
            ...
            # Train Transformer
            model, train_acc, val_acc = train_transformer(
