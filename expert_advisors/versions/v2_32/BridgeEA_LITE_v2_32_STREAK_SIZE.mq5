//+------------------------------------------------------------------+
//|                           BridgeEA_LITE_v2_32_STREAK_SIZE.mq5      |
//|                                            AI Trading System      |
//|     STREAK-BASED SIZING & PYRAMIDING UPDATE                       |
//+------------------------------------------------------------------+
#property copyright "AI Trading System"
#property link      ""
#property version   "2.32"
#property description "Bridge EA v2.32 - STREAK-BASED SIZING"
#property description "NEW: Win streaks enable pyramiding on pullbacks"
#property description "NEW: Loss streaks reduce max positions"
#property description "KEEP: Progressive trailing from v2.31"
...
input double FixedLotSize = 0.10;        // Fixed lot size (no confidence scaling)
...
//--- STREAK-BASED SIZING PARAMETERS (NEW v2.32)
...
input double LossStreak_Tier1_Mult = 0.70;     // Max positions multiplier at tier 1 (70%)
input double LossStreak_Tier2_Mult = 0.50;     // Max positions multiplier at tier 2 (50%)

//--- PYRAMIDING PARAMETERS (NEW v2.32)
...
input double Pyramid_MinRR = 1.0;              // Minimum R:R on existing position to pyramid
input double Pyramid_PullbackPercent = 50.0;   // Pullback % of move to trigger pyramid (50 = half)
input int Pyramid_MaxPerSymbol = 2;            // Maximum positions per symbol when pyramiding
...
//--- STREAK TRACKING (NEW v2.32)
int g_consecutiveWins = 0;         // Current win streak count
int g_consecutiveLosses = 0;       // Current loss streak count
int g_totalWins = 0;               // Total wins this session
int g_totalLosses = 0;             // Total losses this session
int g_lastKnownHistoryCount = 0;   // For detecting new closed trades
bool g_pyramidingEnabled = false;  // Whether pyramiding is currently active
int g_pyramidTradesOpened = 0;     // Pyramid trades opened this session
int g_streakReducedCount = 0;      // Times max positions was reduced by streak
...
int OnInit()
{
    ...
    //--- Initialize streak tracking (NEW v2.32)
    g_consecutiveWins = 0;
    ...
    g_pyramidTradesOpened = 0;
    g_streakReducedCount = 0;
    
    //--- Log streak sizing settings
    if(EnableStreakSizing)
    {
        Print("  STREAK-BASED SIZING (NEW v2.32):");
        Print("    - Tier 1 (", LossStreak_Tier1_Count, "+ losses): ", (int)(LossStreak_Tier1_Mult * 100), "% max positions");
        Print("    - Tier 2 (", LossStreak_Tier2_Count, "+ losses): ", (int)(LossStreak_Tier2_Mult * 100), "% max positions");
    }
    else
    {
        Print("  STREAK SIZING: DISABLED");
    }
    
    //--- Log pyramiding settings
    if(EnablePyramiding)
    {
        Print("  PYRAMIDING (NEW v2.32):");
        Print("    - Enable after: ", WinStreak_PyramidTrigger, " consecutive wins");
        Print("    - Min R:R to pyramid: ", DoubleToString(Pyramid_MinRR, 1), ":1");
        Print("    - Pullback trigger: ", DoubleToString(Pyramid_PullbackPercent, 0), "% retracement");
        Print("    - Max per symbol: ", Pyramid_MaxPerSymbol, " positions");
    }
    else
    {
        Print("  PYRAMIDING: DISABLED");
    }
...
void OnTimer()
{
    ...
    //--- Update streak tracking (NEW v2.32)
    UpdateStreakTracking();
    
    //--- Check pyramid opportunities (NEW v2.32)
...
//+------------------------------------------------------------------+
//| STREAK TRACKING FUNCTIONS (NEW v2.32)                              |
//+------------------------------------------------------------------+

//+------------------------------------------------------------------+
//| Update win/loss streak by checking closed trades                   |
//+------------------------------------------------------------------+
void UpdateStreakTracking()
{
    ...
    // Only process if new deals appeared
    if(currentHistoryCount <= g_lastKnownHistoryCount)
        return;
    
    // Select history for today (or last 24 hours)
    datetime start_time = TimeCurrent() - 86400;  // Last 24 hours
    ...
    for(int i = g_lastKnownHistoryCount; i < historyTotal; i++)
    {
        ...
        // Only check our trades (by magic number)
        ...
        if(net_profit > 0)
        {
            ...
            // Check if pyramiding should be enabled
            if(EnablePyramiding && g_consecutiveWins >= WinStreak_PyramidTrigger && !g_pyramidingEnabled)
            {
                g_pyramidingEnabled = true;
                Print("╔════════════════════════════════════════════════════════════╗");
                Print("║  [STREAK] PYRAMIDING ENABLED! ", g_consecutiveWins, " consecutive wins");
                Print("║  Will add positions on strategic pullbacks");
                Print("╚════════════════════════════════════════════════════════════╝");
            }
        }
        else if(net_profit < 0)
        {
            ...
            // Disable pyramiding on any loss
            ...
            // Log if loss streak triggers protection
            if(EnableStreakSizing)
            {
                if(g_consecutiveLosses == LossStreak_Tier1_Count)
                {
                    Print("╔════════════════════════════════════════════════════════════╗");
                    Print("║  [STREAK] TIER 1 PROTECTION! ", g_consecutiveLosses, " consecutive losses");
                    Print("║  Max positions reduced to ", (int)(MaxTotalPositions * LossStreak_Tier1_Mult));
                    Print("╚════════════════════════════════════════════════════════════╝");
                }
                else if(g_consecutiveLosses == LossStreak_Tier2_Count)
                {
                    Print("╔════════════════════════════════════════════════════════════╗");
                    Print("║  [STREAK] TIER 2 PROTECTION! ", g_consecutiveLosses, " consecutive losses");
                    Print("║  Max positions reduced to ", (int)(MaxTotalPositions * LossStreak_Tier2_Mult));
                    Print("╚════════════════════════════════════════════════════════════╝");
                }
            }
        }
...
{
    if(!EnableStreakSizing)
        return g_currentMaxPositions;  // Use drawdown-scaled value
    
    int base_max = g_currentMaxPositions;  // Already drawdown-scaled
    
    // Apply loss streak reduction
    if(g_consecutiveLosses >= LossStreak_Tier2_Count)
    {
        int reduced = (int)(base_max * LossStreak_Tier2_Mult);
        if(reduced < 1) reduced = 1;
        return reduced;
    }
    else if(g_consecutiveLosses >= LossStreak_Tier1_Count)
    {
        int reduced = (int)(base_max * LossStreak_Tier1_Mult);
        if(reduced < 1) reduced = 1;
        return reduced;
    }
...
//+------------------------------------------------------------------+
string GetStreakStatsSummary()
{
    string pyramid_status = g_pyramidingEnabled ? "ACTIVE" : "inactive";
    return StringFormat("Streak: W=%d L=%d | Pyramid=%s | Total: %dW/%dL",
        g_consecutiveWins, g_consecutiveLosses, pyramid_status,
        g_totalWins, g_totalLosses);
}


//+------------------------------------------------------------------+
//| PYRAMIDING FUNCTIONS (NEW v2.32)                                   |
//+------------------------------------------------------------------+

//+------------------------------------------------------------------+
//| Check for pyramid opportunities on existing positions              |
//+------------------------------------------------------------------+
void CheckPyramidOpportunities()
{
    ...
    for(int i = 0; i < total; i++)
    {
        ...
        // Calculate current profit in R:R terms
        double initial_risk = 0;
        double current_profit_distance = 0;
        double current_price = 0;
        double max_profit_distance = 0;
        ...
        // Only pyramid if position has reached minimum profit
        if(current_rr < Pyramid_MinRR)
            continue;
        
        // Calculate the high water mark (max profit reached)
        // For simplicity, we track this by checking if price has retraced
        double max_move = initial_risk * (current_rr + 0.5);  // Estimate max move
        
        // Check if price has pulled back to pyramid level
        double pullback_distance = max_move * (Pyramid_PullbackPercent / 100.0);
        double pyramid_trigger_price = 0;
        bool trigger_hit = false;
        
        if(pos_type == POSITION_TYPE_BUY)
        {
            // For buy: pullback means price came down from high
            pyramid_trigger_price = entry_price + (max_move - pullback_distance);
            trigger_hit = (current_price <= pyramid_trigger_price && current_price > entry_price);
        }
        else // SELL
        {
            // For sell: pullback means price went up from low
            pyramid_trigger_price = entry_price - (max_move - pullback_distance);
            trigger_hit = (current_price >= pyramid_trigger_price && current_price < entry_price);
        }
        
        // If trigger hit, add pyramid position
        if(trigger_hit)
        {
            ...
            {
                Print("[PYRAMID] Skipped - at max positions (", streak_max, ")");
                continue;
            }
            
            // Open pyramid position with same SL
            double lot_size = FixedLotSize;  // Use standard lot size
            double new_tp = 0;
            
            // Set TP at same distance as original
            ...
            if(success)
            {
                ...
                Print("╔════════════════════════════════════════════════════════════╗");
                Print("║  [PYRAMID] Added position on ", symbol);
                Print("║  Original R:R: ", DoubleToString(current_rr, 1), ":1");
                Print("║  Entry: ", DoubleToString(current_price, digits));
                Print("║  SL: ", DoubleToString(current_sl, digits), " (same as original)");
                Print("║  TP: ", DoubleToString(new_tp, digits));
                Print("║  Pyramid trades this session: ", g_pyramidTradesOpened);
                Print("╚════════════════════════════════════════════════════════════╝");
            }
            else
            {
                Print("[PYRAMID] FAILED for ", symbol, " | Error: ", GetLastError());
            }
        }
    }
}
...
void ProcessTradeCommands()
{
    ...
    //--- Skip header (7 columns: symbol, action, confidence, sl_price, tp_price, lot_size, timestamp)
    FileReadString(handle); FileReadString(handle);
    FileReadString(handle); FileReadString(handle);
    FileReadString(handle); FileReadString(handle);
    FileReadString(handle);  // v5.1: lot_size column added
    ...
    while(!FileIsEnding(handle))
    {
        ...
        double lot_size = StringToDouble(FileReadString(handle));  // v5.1: Read lot_size
        ...
        if(action == "BUY" || action == "SELL")
            ...
            success = ExecuteScaleOut(symbol, confidence, lot_size);  // v5.1: Pass lot_size
...
//+------------------------------------------------------------------+
//| Execute SCALE_OUT - Partial close (v5.1: Quarter-Level Scaling)    |
//+------------------------------------------------------------------+
bool ExecuteScaleOut(string symbol, double confidence, double requested_lot_size = 0.05)
{
    ...
    // v5.1: Use requested lot size from Python, default to 0.05 for quarter-level scaling
    double scale_volume = (requested_lot_size > 0) ? requested_lot_size : 0.05;
    
    // Ensure we don't close more than position has
    ...
    request.comment = "SCALE_OUT_Q";  // Q = Quarter level
    ...
    LogTrade(symbol, "SCALE_OUT", scale_volume, confidence, request.price, 0, 0, 
             result.order, success ? "SUCCESS" : "FAILED", result.retcode, "QUARTER_LEVEL");
    
    if(success)
        Print("[SCALE_OUT SUCCESS] ", symbol, " closed ", scale_volume, " lots at quarter level");
...
//| UPDATED v2.32: Uses streak-adjusted max positions                  |
//+------------------------------------------------------------------+
bool ExecuteTrade(string symbol, string action, double confidence, double sl_price_in = 0, double tp_price_in = 0)
{
    ...
    //--- GET STREAK-ADJUSTED MAX POSITIONS (UPDATED v2.32)
    //--- This already factors in drawdown scaling from g_currentMaxPositions
    ...
    {
        Print("╔════════════════════════════════════════════════════════════╗");
        Print("║  ⛔ TRADING STOPPED - Drawdown OR Loss Streak              ║");
        Print("╠════════════════════════════════════════════════════════════╣");
    ...
    //--- CHECK MAX TOTAL POSITIONS (using streak-adjusted value)
    int current_positions = GetOurPositionCount();
    if(current_positions >= scaled_max)
    {
        string reject_reason = "";
        if(g_consecutiveLosses >= LossStreak_Tier1_Count)
            reject_reason = StringFormat("STREAK_LIMIT_%dL_%d/%d", g_consecutiveLosses, current_positions, scaled_max);
        else
            reject_reason = StringFormat("MAX_POS_TIER%d_%d/%d", g_currentDrawdownTier, current_positions, scaled_max);
        
        Print("[REJECTED] ", symbol, " - Positions: ", current_positions, "/", scaled_max,
              " (DD=", DoubleToString(g_effectiveDrawdownPercent, 2), "%",
              " | Streak: ", g_consecutiveWins, "W/", g_consecutiveLosses, "L)");
        LogTrade(symbol, action, 0, confidence, 0, 0, 0, 0, "REJECTED", 0, reject_reason);
        return false;
    }
