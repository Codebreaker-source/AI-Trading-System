//+------------------------------------------------------------------+
//|                           BridgeEA_LITE_v2_34_EXIT_MGMT.mq5         |
//|                                            AI Trading System      |
...
#property description "Bridge EA v2.34 - EXIT MANAGEMENT OPTIMIZATION"
...
input double FixedLotSize = 0.10;        // v2.34: Synced        // Fixed lot size (SYNCED with Python)
...
input double BE_TriggerRR = 1.5;         // v2.34: Was 0.25        // BE trigger at this R:R (0.25 = 25% of risk)
...
//--- MULTI-STAGE TP PARAMETERS (v2.34 NEW)
input bool EnableMultiStageTP = true;          // v2.34: Enable multi-stage profit taking
input double MultiTP_Stage1_RR = 1.0;          // Close 25% at 1.0 R:R
input double MultiTP_Stage1_Pct = 25.0;        // Percent to close at stage 1
input double MultiTP_Stage2_RR = 1.5;          // Close 25% at 1.5 R:R  
input double MultiTP_Stage2_Pct = 25.0;        // Percent to close at stage 2
input double MultiTP_Stage3_RR = 2.0;          // Close 25% at 2.0 R:R
input double MultiTP_Stage3_Pct = 25.0;        // Percent to close at stage 3

//--- PROGRESSIVE TRAILING PARAMETERS (FROM v2.31)
input bool EnableProgressiveTrail = true;      // Enable progressive trailing
input double ProgTrail_Tier1_RR = 0.5;         // v2.34: Was 1.0         // R:R threshold for tier 1
input double ProgTrail_Tier2_RR = 1.0;         // v2.34: Was 1.5         // R:R threshold for tier 2
input double ProgTrail_Tier3_RR = 1.5;         // v2.34: Was 2.0         // R:R threshold for tier 3
input double ProgTrail_Mult_Tier1 = 0.9;       // Trail multiplier at tier 1
input double ProgTrail_Mult_Tier2 = 0.70;      // v2.34: Was 0.75      // Trail multiplier at tier 2
input double ProgTrail_Mult_Tier3 = 0.5;       // Trail multiplier at tier 3
input double ProgTrail_Tier4_RR = 2.0;         // v2.34: NEW - Tier 4 threshold
input double ProgTrail_Mult_Tier4 = 0.40;      // v2.34: NEW - very tight
...
//--- MULTI-STAGE TP TRACKING (v2.34 NEW)
struct MultiStageTPState {
    ulong ticket;
    bool stage1_done;
    bool stage2_done;
    bool stage3_done;
...
MultiStageTPState g_multiStageTP[50];
int g_multiStageTPCount = 0;
int g_multiStageTP_Stage1_Total = 0;
int g_multiStageTP_Stage2_Total = 0;
int g_multiStageTP_Stage3_Total = 0;
...
int OnInit()
{
    ...
    Print("SUCCESS: BridgeEA v2.34 initialized");
...
double GetProgressiveTrailModifier(double current_rr)
{
    ...
    // v2.34: 4-tier progressive trail
    if(current_rr >= ProgTrail_Tier4_RR)
        return ProgTrail_Mult_Tier4;
    else if(current_rr >= ProgTrail_Tier3_RR)
...
void OnTimer()
{
    ...
    if(EnableBreakEven || EnableTrailing || EnablePartialTP || EnableMultiStageTP)
    {
        datetime now = TimeCurrent();
        if(now - g_lastTrailCheck >= TrailCheckSeconds)
        {
            if(EnableMultiStageTP) ManageMultiStageTP();
...
void OnDeinit(const int reason)
{
    ...
    Print("  Multi-Stage TP: S1=", g_multiStageTP_Stage1_Total, " S2=", g_multiStageTP_Stage2_Total, " S3=", g_multiStageTP_Stage3_Total);
    Print("  Calendar Events: ", g_calendarEventsExported);
}
...
bool ExecuteTrade(string symbol, string action, double confidence, double sl_price_in = 0, double tp_price_in = 0)
{
    ...
    request.comment = StringFormat("AI_%.0f%%_v2.34", confidence * 100);
...
//+------------------------------------------------------------------+
//| Manage Multi-Stage Take Profit (v2.34 NEW)                         |
//+------------------------------------------------------------------+
void ManageMultiStageTP()
{
    if(!EnableMultiStageTP) return;
    ...
    for(int i = 0; i < total; i++)
    {
        ...
        int idx = -1;
        for(int j = 0; j < g_multiStageTPCount; j++) {
            if(g_multiStageTP[j].ticket == ticket) { idx = j; break; }
        }
        if(idx < 0 && g_multiStageTPCount < 50) {
            idx = g_multiStageTPCount++;
            g_multiStageTP[idx].ticket = ticket;
            g_multiStageTP[idx].stage1_done = false;
            g_multiStageTP[idx].stage2_done = false;
            g_multiStageTP[idx].stage3_done = false;
            g_multiStageTP[idx].original_volume = PositionGetDouble(POSITION_VOLUME);
        }
        if(idx < 0) continue;
        ...
        double initial_risk = 0, current_profit_distance = 0;
        if(pos_type == POSITION_TYPE_BUY) {
        ...
        double min_lot = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MIN);
        double orig_vol = g_multiStageTP[idx].original_volume;
        
        // Stage 1: Close 25% at 1.0 R:R
        if(!g_multiStageTP[idx].stage1_done && current_rr >= MultiTP_Stage1_RR) {
            double close_vol = NormalizeDouble(orig_vol * MultiTP_Stage1_Pct / 100.0, 2);
            if(close_vol >= min_lot && volume > close_vol) {
                if(g_trade.PositionClosePartial(ticket, close_vol)) {
                    g_multiStageTP[idx].stage1_done = true;
                    g_multiStageTP_Stage1_Total++;
                    Print("[MULTI-TP] Stage 1: ", symbol, " closed ", close_vol, " @ ", DoubleToString(current_rr, 2), "R");
                }
            }
        }
        
        // Stage 2: Close 25% at 1.5 R:R
        if(!g_multiStageTP[idx].stage2_done && current_rr >= MultiTP_Stage2_RR) {
            double close_vol = NormalizeDouble(orig_vol * MultiTP_Stage2_Pct / 100.0, 2);
            if(close_vol >= min_lot && volume > close_vol) {
                if(g_trade.PositionClosePartial(ticket, close_vol)) {
                    g_multiStageTP[idx].stage2_done = true;
                    g_multiStageTP_Stage2_Total++;
                    Print("[MULTI-TP] Stage 2: ", symbol, " closed ", close_vol, " @ ", DoubleToString(current_rr, 2), "R");
                }
            }
        }
        
        // Stage 3: Close 25% at 2.0 R:R
        if(!g_multiStageTP[idx].stage3_done && current_rr >= MultiTP_Stage3_RR) {
            double close_vol = NormalizeDouble(orig_vol * MultiTP_Stage3_Pct / 100.0, 2);
            if(close_vol >= min_lot && volume > close_vol) {
                if(g_trade.PositionClosePartial(ticket, close_vol)) {
                    g_multiStageTP[idx].stage3_done = true;
                    g_multiStageTP_Stage3_Total++;
                    Print("[MULTI-TP] Stage 3: ", symbol, " closed ", close_vol, " @ ", DoubleToString(current_rr, 2), "R");
                }
            }
        }
        // Remaining 25% trails to maximize profit
    }
}
